...
def main():
    ...
    if app_mode == "比較":
        ...
        with st.expander("このビューアについて", expanded=True):
            st.markdown("""
            観光客と地域住民では、同じ街であっても「実際に利用できる空間」が異なります。
            このビューアでは、地図上で矩形を描いてエリアを選ぶとその違いを3Dで**可視化**し**定量的に比較**することができます。
            
            まず、建物の**高さ**と**色**の組み合わせから、エリアの性格を視覚的に読み取ることができます。
            高さによって、そのエリアの建物の規模を読み取ることができます。
            色によって、
            - **暖色系（商業ビルや公園、駅など）**：観光客・地域住民の双方が自由に利用できる空間
            - **寒色系（住居や農地など）**：通常は地域住民しか利用しない空間
            が把握できます（詳細は画面右端の凡例を参照）。

            こうした視覚的な特徴を定量的に表したものが**補正係数α**です。
            αが大きいほど地域色が強く、小さいほど観光色が強いエリアであると分析できます。
            """)
